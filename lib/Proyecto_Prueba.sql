...
DROP TRIGGER IF EXISTS validar_comprobante_update;
DROP TRIGGER IF EXISTS validar_comprobante_insert;
...
CREATE TRIGGER validar_comprobante_insert
BEFORE INSERT ON comprobantes_movimientos
FOR EACH ROW
BEGIN
  IF NEW.tipo_comprobante = 'factura' AND (NEW.numero_referencia IS NULL OR NEW.numero_referencia = '') THEN
    SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'Una factura debe tener un número de referencia';
...
-- Función mejorada para encriptar contraseñas
...
BEGIN
  ...
  RETURN CONCAT(salt, ':', SHA2(CONCAT(p_contraseña, salt), 512));
END //
...
-- Procedimiento para leer empleados con usuarios (CORREGIDO)
...
BEGIN
  ...
  SELECT contraseña_usuario INTO v_stored_hash
  FROM usuarios 
  WHERE nombre_usuario = p_nombre_usuario;
  
  IF v_stored_hash IS NOT NULL THEN
    IF v_stored_hash LIKE '%:%' THEN
      SET v_salt = SUBSTRING_INDEX(v_stored_hash, ':', 1);
      SET p_existe = CONCAT(v_salt, ':', SHA2(CONCAT(p_contraseña, v_salt), 512)) = v_stored_hash;
    ELSE
      SET p_existe = v_stored_hash = p_contraseña;
    END IF;
  ELSE
    SET p_existe = FALSE;
  END IF;
...
BEGIN
  ...
  SELECT Contraseña INTO v_stored_hash
  FROM administrador 
  WHERE NombreAdmin = p_nombre_admin;
  
  IF v_stored_hash IS NOT NULL THEN
    -- Si es un hash nuevo (contiene ':')
    IF v_stored_hash LIKE '%:%' THEN
      SET v_salt = SUBSTRING_INDEX(v_stored_hash, ':', 1);
      SET v_calculated_hash = SHA2(CONCAT(p_contraseña, v_salt), 512);
      SET p_existe = CONCAT(v_salt, ':', v_calculated_hash) = v_stored_hash;
    ELSE
      -- Compatibilidad con contraseñas antiguas (comparación directa)
      SET p_existe = v_stored_hash = p_contraseña;
    END IF;
  ELSE
    SET p_existe = FALSE;
...
-- Verificar que la referencia exista y tenga un estado válido
IF p_tipo_contrato = 'venta' THEN
    -- Para ventas, verificar que esté en proceso (7) o completada (8)
    SELECT COUNT(*) INTO v_referencia_existe
    FROM ventas 
    WHERE id_venta = p_id_referencia AND id_estado IN (7, 8);
    
    IF v_referencia_existe = 0 THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'La venta especificada no existe o no tiene un estado válido para generar contrato';
    END IF;
ELSE
    -- Para contratos de renta, verificar que esté activo (estado 1)
    SELECT COUNT(*) INTO v_referencia_existe
    FROM contratos_renta 
    WHERE id_contrato = p_id_referencia AND id_estado = 1;
    
    IF v_referencia_existe = 0 THEN
        SIGNAL SQLSTATE '45000' SET MESSAGE_TEXT = 'El contrato de renta especificado no existe o no está activo';
    END IF;
END IF;

-- Determinar la versión del contrato
